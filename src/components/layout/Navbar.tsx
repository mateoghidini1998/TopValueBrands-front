...
const Navbar = () => {
  ...
  return (
    <nav
      ...
      ${
        sidebarOpen ? "w-[16rem] duration-[0.6s] ease-in-out" : "w-[3.75rem] duration-[0.6s] ease-in-out"
    ...
    >
      <div className="text-lg font-bold px-12 pb-[30px] mb-[20px] pt-9 text-center h-[110px] text-nowrap">
        <h1 className={` ${!sidebarOpen ? "hidden" : ""}`}>Top Value Brands</h1>
      </div>
      {/* Logo */}

      {/* Nav Items */}
      <div className="flex flex-col items-center w-full">
        <Menu isOpen={sidebarOpen} setIsOpen={setSidebarOpen} links={filteredLinks} />
      </div>

      {/* Toggle Nav */}
      {/* center the button on axis X */}

      <div className={`bottom_buttons absolute w-full h-[100px] bottom-4 flex justify-between px-4 items-center ${!sidebarOpen && "flex-row"}`}>
        {/* <div className="bottom-4 left-4">
        </div> */}
        <button onClick={() => {toggleSidebar()}} className={`bottom-4 right-4 ${!sidebarOpen ? "rotate-180 duration-800 ease-in-out" : "rotate-[-180] duration-800 ease-in-out"}`}>
