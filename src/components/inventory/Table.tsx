...
const Table: FC = () => {
  ...
  return (
    <>
      <button onClick={() => handleDelete("09-8SNW-SMV2")}>Delete</button>
      {ConfirmAlert}
      <table className={`w-full mb-5 relative`}>

        {/* Blur Background Div */}
        {ConfirmAlert && (
          <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 backdrop-filter backdrop-blur-md flex justify-center items-center z-[1100]">
          </div>
        )}
